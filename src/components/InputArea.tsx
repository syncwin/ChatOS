...
const InputArea = ({
  ...
  return <div className="py-4 bg-background/90 backdrop-blur-sm">
      <form onSubmit={onSubmit} className="flex gap-4 items-end">
        <div className="flex-1 flex items-center p-2 bg-input border border-input focus-within:ring-2 focus-within:ring-primary transition-all px-[4px] py-[4px] rounded">
            ...
            <TextareaAutosize id="chat-input" value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Ask me anything..." disabled={isLoading} maxRows={5} minRows={1} className="flex-1 w-full resize-none p-2 bg-transparent text-base focus:outline-none rounded-none px-[4px] py-0" />
