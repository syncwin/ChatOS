  ...
  return <Collapsible open={!isCollapsed} onOpenChange={onToggle}>
      <div className="flex items-center justify-between px-2 py-1 group/section-header hover:bg-muted/50 rounded-md transition-colors">
        <CollapsibleTrigger asChild>
          <Button variant="ghost" size="sm" className="flex items-center gap-2 text-xs font-medium text-muted-foreground hover:text-foreground p-1 h-auto py-[4px] text-center px-[4px] hover:bg-transparent">
        ...
        <div className="flex items-center gap-1">
          {rightElement}
          <Button variant="ghost" size="icon" className="h-6 w-6 opacity-0 group-hover/section-header:opacity-100 transition-opacity hover:bg-muted" onClick={e => {
          e.stopPropagation();
          onTogglePin();
